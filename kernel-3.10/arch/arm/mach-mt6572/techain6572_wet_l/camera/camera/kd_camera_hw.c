...
#ifdef GPIO_MAIN_CAMERA_28V_POWER_CTRL_PIN
...
int kdCISModulePowerOn(CAMERA_DUAL_CAMERA_SENSOR_ENUM SensorIdx, char *currSensorName, BOOL On, char* mode_name)
{
    ...
    if (On) {
       ...
       if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_T8EV5_YUV,currSensorName)))
       {
		   PK_DBG("[CAMERA SENSOR] kdCISModulePowerOn get in---T8EV5_YUV sensorIdx:%d; pinSetIdx=%d\n",SensorIdx, pinSetIdx);
       ...
       {
		   PK_DBG("[CAMERA SENSOR] kdCISModulePowerOn get in---SP2519_YUV sensorIdx:%d; pinSetIdx=%d\n",SensorIdx, pinSetIdx);
	           //PDN pin
		  if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_MODE])){PK_DBG("[CAMERA LENS] set gpio mode failed!! \n");}
		  if(mt_set_gpio_dir(pinSet[pinSetIdx][IDX_PS_CMPDN],GPIO_DIR_OUT)){PK_DBG("[CAMERA LENS] set gpio dir failed!! \n");}
                  if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMRST],pinSet[pinSetIdx][IDX_PS_CMRST+IDX_PS_MODE])){PK_DBG("[CAMERA SENSOR] set gpio mode failed!! \n");}
		  ...
		  if(mt_set_gpio_out(pinSet[pinSetIdx][IDX_PS_CMRST],pinSet[pinSetIdx][IDX_PS_CMRST+IDX_PS_OFF])){PK_DBG("[CAMERA SENSOR] set gpio failed!! \n");}
		   ...
		   if(TRUE != hwPowerOn(CAMERA_POWER_SUB_VCAM_D, VOL_1800,mode_name))
		   {
			   PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
			   //return -EIO;
			   goto _kdCISModulePowerOn_exit_;
		   ...
		   if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST]) 
		   {
			 ...
			 msleep(3);
	                 if(mt_set_gpio_out(pinSet[pinSetIdx][IDX_PS_CMPDN],0)){PK_DBG("[CAMERA SENSOR]  set gpio failed!! \n");}
                         msleep(1);
			 //RST pin
	 ...
	 if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_SP2519_YUV,currSensorName)))
         {
       ...
       if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_SP2519_YUV,currSensorName)))
       {
           if(TRUE != hwPowerDown(CAMERA_POWER_SUB_VCAM_D, mode_name)) {
