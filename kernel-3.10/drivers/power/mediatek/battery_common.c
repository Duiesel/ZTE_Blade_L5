...
#include "cust_pe.h"
#endif
#define ZTE_CUSTOM_FULL_VOL 4260
/* ////////////////////////////////////////////////////////////////////////////// */
...
static kal_uint32 bq24158_chip_status;
static kal_uint32 bq24158_fault_status;
/* ////////////////////////////////////////////////////////////////////////////// */
/* FOR ADB CMD */
/* ////////////////////////////////////////////////////////////////////////////// */
/* Dual battery */
				...
				enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ChargerVoltage:
		...
		val->intval = data->BAT_chg_current;
		break;
		/* Dual battery */
...
static void mt_battery_update_EM(struct battery_data *bat_data)
{
	...
	bat_data->BAT_ChargerVoltage = BMT_status.charger_vol;
        bat_data->BAT_chg_current = battery_meter_get_charging_current();
	/* Dual battery */
	bat_data->status_smb = g_status_smb;
	bat_data->capacity_smb = g_capacity_smb;
	bat_data->present_smb = g_present_smb;
        battery_log(BAT_LOG_CRTI, "chg_current = %d\n",bat_data->BAT_chg_current);
...
static kal_bool mt_battery_100Percent_tracking_check(void)
{
	...
	if (BMT_status.bat_full == KAL_TRUE)	/* charging full first, UI tracking to 100% */
	{
		//if (BMT_status.UI_SOC >= 100) { 
	        if (BMT_status.bat_vol >= ZTE_CUSTOM_FULL_VOL) {  //vbat >4.26V--->100%
			...
			if (timer_counter >= (cust_sync_time / BAT_TASK_PERIOD) && BMT_status.UI_SOC <100) {
				timer_counter = 1;
				BMT_status.UI_SOC++;
			} else {
				timer_counter++;
...
static void mt_battery_Sync_UI_Percentage_to_Real(void)
{
	...
	if ((BMT_status.UI_SOC > BMT_status.SOC) && ((BMT_status.UI_SOC != 1))) {
              if(BMT_status.bat_vol >= ZTE_CUSTOM_FULL_VOL)
                   return;
		...
		if (BMT_status.UI_SOC == -1)
			...
			if (BMT_status.UI_SOC < BMT_status.SOC
			    && (BMT_status.SOC - BMT_status.UI_SOC > 1) && BMT_status.UI_SOC<100)
				BMT_status.UI_SOC++;
			else
				BMT_status.UI_SOC = BMT_status.SOC;
		}
...
PMU_STATUS do_batt_temp_state_machine(void)
{
	...
	if (batt_cust_data.bat_low_temp_protect_enable)
	{
		...
		g_batt_temp_status = TEMP_POS_LOW;
                mt65xx_leds_brightness_set(MT65XX_LED_TYPE_RED,0); //red off 
                if(BMT_status.UI_SOC==100)
                    mt65xx_leds_brightness_set(MT65XX_LED_TYPE_GREEN,0);   //green off  
                 else
                    mt65xx_leds_brightness_set(MT65XX_LED_TYPE_RED_1,0); //red_1 off  
		return PMU_STATUS_FAIL;
			...
			battery_log(BAT_LOG_CRTI,
					    "[BATTERY] Battery Temperature raise from %d to %d(%d), allow charging!!\n\r",
                         ...
                         if(BMT_status.UI_SOC==100)
                 		mt65xx_leds_brightness_set(MT65XX_LED_TYPE_GREEN,255);   //green on
...
void mt_battery_GetBatteryData(void)
{
	...
	static kal_int32 previous_SOC = -1;
        ...
        bq24158_chip_status=bq24158_get_chip_status();
        bq24158_fault_status=bq24158_get_fault_status();
...
static PMU_STATUS mt_battery_CheckChargerVoltage(void)
{
	...
	if (BMT_status.charger_exist == KAL_TRUE) {
		...
		#if !defined(CONFIG_MTK_DUAL_INPUT_CHARGER_SUPPORT)
		if (BMT_status.charger_vol >=ZTE_OVP_VALUE /*batt_cust_data.v_charger_max*/ || (bq24158_chip_status ==3 && bq24158_fault_status== 1) ) {
		#else
...
static void mt_battery_notify_VBatTemp_check(void)
{
	...
	if (BMT_status.temperature >= 60) {
		g_BatteryNotifyCode |= 0x0040;
		battery_log(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) out of range(too high 60)\n",
				    BMT_status.temperature);
	}
...
void mt_battery_notify_check(void)
{
	...
	//	if (BMT_status.charger_exist == KAL_TRUE)  //check high temperature for only charging mode.
                {
...
void do_chrdet_int_task(void)
{
	...
	if (g_bat_init_flag == KAL_TRUE) {
		...
		if (BMT_status.bat_vol > 0) {
			if(BMT_status.charger_exist == KAL_TRUE && BMT_status.bat_full != KAL_TRUE && BMT_status.charger_vol<ZTE_OVP_VALUE)
                            battery_charging_control(CHARGING_CMD_ENABLE,&BMT_status.charger_exist); 
			mt_battery_update_status();
		...
		if ((upmu_is_chr_det() == KAL_TRUE)) {
			check_battery_exist();
                        if(BMT_status.UI_SOC>90)
                        {
                          	...
                          	{
		                  cap_90 = 1;
		                  mt65xx_leds_brightness_set(MT65XX_LED_TYPE_RED_1,0);
                          	}
                        }
		}
#if 0 //add by andy
                ...
                {	
                        if(BMT_status.UI_SOC<5 )
		        {
			   //ktime = ktime_set(0, BAT_MS_TO_NS(3000));
                          if(cap_10==0)
                          {
                           cap_10 = 1;
			   mt65xx_leds_brightness_set(MT65XX_LED_TYPE_RED,255);   //isink breath on
		          }
		          else
		          {
		             mt65xx_leds_brightness_set(MT65XX_LED_TYPE_RED,0);   //isink breath on	
		             cap_10 = 0;
		          }
		        }
		        else if(cap_10 == 1)
                        {
	                   mt65xx_leds_brightness_set(MT65XX_LED_TYPE_RED,0);   //isink1 off    
                           cap_10 = 0;                       
                        }
		}
#endif
