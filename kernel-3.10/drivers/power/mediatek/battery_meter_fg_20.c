...
INT32 oam_v_ocv;
INT32 oam_r;
INT32 swfg_ap_suspend_time;
INT32 ap_suspend_car;
struct timespec ap_suspend_time;
INT32 total_suspend_times=0;
INT32 this_suspend_times=0;
INT32 last_hwocv;
INT32 last_i;
kal_int32 hwocv_token;
kal_int32 is_hwocv_update=KAL_FALSE;
...
int battery_meter_get_low_battery_interrupt_status(void)
{
	...
	battery_meter_ctrl(BATTERY_METER_CMD_GET_LOW_BAT_INTERRUPT_STATUS, &is_lbat_int_trigger);
	...
	/* -- end of hibernation path */
...
#if defined(SOC_BY_SW_FG)
		{
			...
			if(battery_meter_get_low_battery_interrupt_status()==KAL_TRUE)
				battery_meter_ctrl(BATTERY_METER_CMD_SET_LOW_BAT_INTERRUPT, &oam_v_ocv);
