...
static s32 i2c_dma_write_mtk(u16 addr, u8 * buffer, s32 len)
{
	...
	while (pos != len) {
		if (len - pos > (IIC_DMA_MAX_TRANSFER_SIZE - FTS_ADDR_LENGTH)) {
			transfer_length = IIC_DMA_MAX_TRANSFER_SIZE - FTS_ADDR_LENGTH;
		} else {
		...
		memcpy(&gpDMABuf_va[FTS_ADDR_LENGTH], &buffer[pos], transfer_length);

		msg.len = transfer_length + FTS_ADDR_LENGTH;
...
static s32 i2c_write_mtk(u16 addr, u8 * buffer, s32 len)
{
	...
	struct i2c_msg msg = {
		.flags = 0,
		.addr = (i2c_client->addr & I2C_MASK_FLAG) | (I2C_ENEXT_FLAG),	//remain
		.timing = I2C_MASTER_CLOCK,
	};
...
{
	int ret;
	u8 addr_buf[FTS_ADDR_LENGTH] = { (addr >> 8) & 0xFF, addr & 0xFF };
...
#if 0
s32 fts_i2c_Read(struct i2c_client *client,char *writebuf,int writelen, char *readbuf, int readlen)
{
     s32 ret;
     u8 reg=*writebuf;

     if(writelen==1)
     ret=ft_i2c_write(reg,NULL,0);
     else
     ret=ft_i2c_write(reg,writebuf,writelen);
     if(ret<0)
       printk("%s error!",__func__);

     ret=ft_i2c_write(reg,readbuf,readlen);
     if(ret<0)
       printk("%s error!",__func__);
...
static unsigned char CTPM_FW[] = {
	//#include "FT6336S_W86H_CBF4948-4.5_Ver0X13_20150418_app.i"
};
...
int fts_create_sysfs(struct i2c_client *client)
{
	...
	printk("FTP: I2CDMABuf_pa=%x\n",(unsigned int )I2CDMABuf_pa);
...
#else
	if (gpDMABuf_va) {
		dma_free_coherent(NULL, IIC_DMA_MAX_TRANSFER_SIZE, gpDMABuf_va, gpDMABuf_va);
		gpDMABuf_pa = 0;
		gpDMABuf_va = NULL;
	}
#endif
