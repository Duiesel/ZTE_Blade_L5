...
s32 DrvMainTouchDeviceInitialize(void)
{
    ...
    if (g_ChipType != 0) // To make sure TP is attached on cell phone.
    {
...
#ifdef CONFIG_ENABLE_CHARGER_DETECTION 
        {
            ...
            if (strstr(szChargerStatus, "Charging") != NULL || strstr(szChargerStatus, "Fully charged") != NULL || strstr(szChargerStatus, "Full") != NULL)
            {
...
static s32 _DrvMainCreateProcfsDirEntry(void)
{
    ...
    if (NULL == proc_create(PROC_NODE_OPEN_GLOVE_MODE, PROCFS_AUTHORITY, _gProcDevice, &_gProcOpenGloveMode))
    {
    ...
    if (NULL == proc_create(PROC_NODE_CLOSE_GLOVE_MODE, PROCFS_AUTHORITY, _gProcDevice, &_gProcCloseGloveMode))
    {
