...
static u8 pd_cal(HANDLE handle, DEVICEID devid, u32 freq, u8 *pd_cal_table)
{
	if (devid == DIV_BROADCAST) {
		...
		fc8300_tuner_set_pll(handle, DIV_SLAVE0, freq, 0);
		fc8300_tuner_set_pll(handle, DIV_SLAVE1, freq, 0);
		fc8300_tuner_set_pll(handle, DIV_SLAVE2, freq, 0);
	} else {
	...
	if (devid == DIV_BROADCAST) {
		...
		fc8300_read(handle, DIV_SLAVE0, 0x98, &pd_cal_table[1]);
		fc8300_read(handle, DIV_SLAVE1, 0x98, &pd_cal_table[2]);
		fc8300_read(handle, DIV_SLAVE2, 0x98, &pd_cal_table[3]);
	} else {
		...
		enum BROADCAST_TYPE broadcast)
{
	...
	if (devid == DIV_BROADCAST) {
		for (i = 0; i < 30; i++) {
			...
			fc8300_read(handle, DIV_SLAVE0, 0x43, &filter[1]);
			fc8300_read(handle, DIV_SLAVE1, 0x43, &filter[2]);
			fc8300_read(handle, DIV_SLAVE2, 0x43, &filter[3]);

			if (((filter[0] >> 4) & 0x01) == 1)
				filter_check += 1;

			if (((filter[1] >> 4) & 0x01) == 1)
				filter_check += 1;

			if (((filter[2] >> 4) & 0x01) == 1)
				filter_check += 1;

			if (((filter[3] >> 4) & 0x01) == 1)
				filter_check += 1;
	...
	if (devid == DIV_BROADCAST) {
		...
		vco_cal(handle, DIV_SLAVE0, 0);
		vco_cal(handle, DIV_SLAVE1, 0);
		vco_cal(handle, DIV_SLAVE2, 0);
	} else {
	...
	if ((broadcast_type == ISDBT_13SEG) || (broadcast_type == ISDBT_1SEG) ||
		...
		if (devid == DIV_BROADCAST) {
			...
			tf_value[1] = (tf_cal(handle, DIV_SLAVE0) - 7);
			tf_value[2] = (tf_cal(handle, DIV_SLAVE1) - 7);
			tf_value[3] = (tf_cal(handle, DIV_SLAVE2) - 7);
		} else {
	...
	if (tf_value[0] != 0) {
		if (tf_offset[0][0] > freq) {
			...
			if (devid != DIV_BROADCAST) {
				...
				tf_cal_set = tf_value[1] + tf_offset[1][i];
				...
				tf_cal_set = tf_value[2] + tf_offset[1][i];
				...
				tf_cal_set = tf_value[3] + tf_offset[1][i];
