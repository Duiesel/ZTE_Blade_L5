		...
		enum BROADCAST_TYPE broadcast)
{
	...
	if (devid == DIV_BROADCAST) {
		for (i = 0; i < 30; i++) {
			...
			fc8300b_read(handle, DIV_SLAVE0, 0x43, &filter[1]);
			fc8300b_read(handle, DIV_SLAVE1, 0x43, &filter[2]);
			fc8300b_read(handle, DIV_SLAVE2, 0x43, &filter[3]);
...
s32 fc8300b_set_freq(HANDLE handle, DEVICEID devid, u32 freq)
{
	...
	if (tf_value[0] != 0) {
		if (tf_offset[0][0] > freq) {
			...
			if (devid != DIV_BROADCAST) {
				...
				fc8300b_write(handle, DIV_SLAVE0, 0x1c,
							tf_cal_set);
				...
				fc8300b_write(handle, DIV_SLAVE1, 0x1c,
							tf_cal_set);
				...
				fc8300b_write(handle, DIV_SLAVE2, 0x1c,
							tf_cal_set);
			}
		}
	}
	...
	if (devid != DIV_BROADCAST) {
		...
		fc8300b_tuner_set_pll(handle, DIV_SLAVE0, freq, offset);
		fc8300b_tuner_set_pll(handle, DIV_SLAVE1, freq, offset);
		fc8300b_tuner_set_pll(handle, DIV_SLAVE2, freq, offset);
	}
