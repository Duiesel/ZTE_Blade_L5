...
/*----------------------------------------------------------------------------*/
static int LIS3DH_ReadData(struct i2c_client *client, s16 data[LIS3DH_AXES_NUM+1])
{
...
/*----------------------------------------------------------------------------*/
static int LIS3DH_ReadCalibration(struct i2c_client *client, int dat[LIS3DH_AXES_NUM+1])
{
    struct lis3dh_i2c_data *obj = i2c_get_clientdata(client);
...
*/
/*----------------------------------------------------------------------------*/
static int LIS3DH_WriteCalibration(struct i2c_client *client, int dat[LIS3DH_AXES_NUM+1])
{
	struct lis3dh_i2c_data *obj = i2c_get_clientdata(client);
	int err = 0;
...

{
	...
	if(err)
	{
        ...
        // printk("%s cmd:%x,arg=%x,client=%x\n",__func__,cmd,(unsigned int)arg,(unsigned int)client);
        //  return err;
	switch(cmd)
	{
