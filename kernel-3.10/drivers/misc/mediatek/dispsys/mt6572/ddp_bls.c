...
#define MIN_PWM_DUTY (5*1023/100)
...
static unsigned int brightness_mapping(unsigned int level)
{
    ...
    else if(level>0 && level< MIN_PWM_DUTY)
        mapped_level = MIN_PWM_DUTY;
    else mapped_level = level;    
...
{       
    ...
    DISP_REG_SET(DISP_REG_BLS_PWM_CON, 0x0 | (0x1 << 16)); //mtk_add
