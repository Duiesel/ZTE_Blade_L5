...
char flashlight_flag = -1;  //0: torch mode  1: flash mode
int readReg(int reg)
{
    char buf[2];
    char bufR[2];
...
//if (1)
{
     if( g_duty > 1 /*&& flashlight_flag!=1*/)  //flash mode
         // if( duty == 1 && flashlight_flag!=1)  //flash mode
	 {
	   mt_set_gpio_out(GPIO_CAMERA_FLASH_EN_PIN,GPIO_OUT_ZERO);
	    mt_set_gpio_out(GPIO_CAMERA_FLASH_MODE_PIN,GPIO_OUT_ONE); 
	    printk("flash_led:flash mode enable.\n");
	 }
         else  if( g_duty ==1 /*&& (flashlight_flag!=0)*/)   //torch mode
         //else  if( duty ==0 && flashlight_flag!=0)   //torch mode
	 {
                mt_set_gpio_out(GPIO_CAMERA_FLASH_EN_PIN,GPIO_OUT_ONE);
		mt_set_gpio_out(GPIO_CAMERA_FLASH_MODE_PIN,GPIO_OUT_ZERO); 
		printk("flash_led:torch mode enable.\n");
	 }
}
#endif
    return 0;
...
int FL_dim_duty(kal_uint32 duty)
{
	PK_DBG(" FL_dim_duty line=%d\n",__LINE__);
        if( 0/* g_duty != duty*/)
        {
          if( duty > 1)  //flash mode
          {  
           flashlight_flag = 1;
	   mt_set_gpio_out(GPIO_CAMERA_FLASH_EN_PIN,GPIO_OUT_ZERO);
	    mt_set_gpio_out(GPIO_CAMERA_FLASH_MODE_PIN,GPIO_OUT_ONE); 
	    printk("flash_led:flash mode.\n");
         }
         else  if( duty ==1 )   //torch mode
          {
               flashlight_flag = 0;
                mt_set_gpio_out(GPIO_CAMERA_FLASH_EN_PIN,GPIO_OUT_ONE);
		mt_set_gpio_out(GPIO_CAMERA_FLASH_MODE_PIN,GPIO_OUT_ZERO); 
		printk("flash_led:torch mode.\n");
	  }
	}
	g_duty = duty;
    return 0;
