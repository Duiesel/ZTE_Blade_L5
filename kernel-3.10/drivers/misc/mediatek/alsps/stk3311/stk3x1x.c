...
#include "alsps.h"
#include "stk3x1x.h"
#define DRIVER_VERSION          "3.2.2 20140822"
...
#if 0 // (defined(MT6589) || defined(MT6572) || defined(MT6575))
...
extern void mt65xx_eint_registration(kal_uint8 eintno, kal_uint32 Dbounce_En,
		kal_uint32 ACT_Polarity, void (EINT_FUNC_PTR)(void),
		kal_uint32 auto_umask);

#endif


#if 0 //def CUST_EINT_ALS_TYPE
...
#define STK_WRITE_CALI 0
#define STK_WRITE_CALI_PROC 1
...
#if  STK_WRITE_CALI_PROC
static struct proc_dir_entry *stk_cali_proc = NULL;
#define STK_CALI_PROC_FILE "alsps_cali"
#endif

#ifdef STK_TUNE0
// Gionee LINUX_BBL5513B_GNNCR00005198_LIYANG add for psensor param, start
//#define STK_MAX_MIN_DIFF          200	
#define STK_MAX_MIN_DIFF         -1
#define STK_LT_N_CT     10  //20 15
#define STK_HT_N_CT     20  //50 18
// Gionee LINUX_BBL5513B_GNNCR00005198_LIYANG add for psensor param, end
...
#if ((STK_WRITE_CALI==1) || (STK_WRITE_CALI_PROC == 1))
static int cal_finished = 0;
static int PS_MAX_XTALK	= 400;//500;
static int PS_l_offset	= STK_LT_N_CT;//83;//100;
static int PS_h_offset	=  STK_HT_N_CT;//177;//150;
#endif
/*----------------------------------------------------------------------------*/
...
#if ((STK_WRITE_CALI==1) || (STK_WRITE_CALI_PROC==1))
#define PS_CAL_FILE_PATH	"/protect_f/xtalk_cal" ///data/alsps/xtalk_cal      //PS Calbration file path alan

static int write_factory_calibration(struct stk3x1x_priv *data, char* ps_data, int ps_cal_len)
{
    struct file *fp_cal;
	mm_segment_t fs;
	loff_t pos;
	APS_FUN();
    pos = 0;
	fp_cal = filp_open(PS_CAL_FILE_PATH, O_CREAT|O_RDWR|O_TRUNC, 0774);
	if (IS_ERR(fp_cal))
	{
...
static bool read_factory_calibration(struct stk3x1x_priv *obj)
{
	struct file *fp;
	mm_segment_t fs;
	loff_t pos;
	char buffer[100]= {0};
		...
		else
		{
			...
			set_fs(fs);
			atomic_set(&obj->ps_high_thd_val, ps_hthr); 
			atomic_set(&obj->ps_low_thd_val,  ps_lthr); 
	 
			APS_LOG("ALSPS_IOCTL_SET_CALI :  high:%d low:%d\n", atomic_read(&obj->ps_high_thd_val), atomic_read(&obj->ps_low_thd_val)); 
...
static int stk3x1x_enable_ps(struct i2c_client *client, int enable, int validate_reg)
{
	...
	{
		data = 0x30;
		err = stk3x1x_master_send(client, 0x01, &data, 1);
		if (err < 0)
		{
			APS_ERR("%s write back ps_it err = %d\n", __func__,err);
			return -EFAULT;
		} 
		data = 0xff;
		err = stk3x1x_master_send(client, 0x03, &data, 1);
		if (err < 0)
		{
			APS_ERR("%s write back ps_it err = %d\n", __func__,err);
			return -EFAULT;
		...
		data = 0x1;//9
		err = stk3x1x_master_send(client, 0x04, &data, 1);
		if (err < 0)
		{
			APS_ERR("%s write back ps_it err = %d\n", __func__,err);
			return -EFAULT;
		...
		/*
		data = 0xff;
		err = stk3x1x_master_send(client, 0x0a, &data, 1);
		if (err < 0)
		{
			APS_ERR("%s write back ps_it err = %d\n", __func__,err);
			return -EFAULT;
		...
		data = 0xff;
		err = stk3x1x_master_send(client, 0x0b, &data, 1);
		if (err < 0)
		{
			APS_ERR("%s write back ps_it err = %d\n", __func__,err);
			return -EFAULT;
		...
		data = 0x0;
		err = stk3x1x_master_send(client, 0x0c, &data, 1);
		if (err < 0)
		{
			APS_ERR("%s write back ps_it err = %d\n", __func__,err);
			return -EFAULT;
		...
		data = 0x14;
		err = stk3x1x_master_send(client, 0x0d, &data, 1);
		if (err < 0)
		{
			APS_ERR("%s write back ps_it err = %d\n", __func__,err);
			return -EFAULT;
	...
	if(enable)
	{
		...
		obj->psoff_ir = 0;
			...
			err=read_factory_calibration(obj); //alsp_cali version
...
static int32_t stk3x1x_enable_ges(struct i2c_client *client, int enable, int mode)
{
	...
	//if(obj->re_enable_ges2!=0 && enable==0)
	//  obj->re_enable_ges2 = 0;
...
static int stk3x1x_ps_val(void)
{
	...
	//ret = stk3x1x_master_recv(stk3x1x_obj->client, 0x20, buf, 4);
        ret = stk3x1x_master_recv(stk3x1x_obj->client, stk3x1x_obj->addr.data1_offset, buf, 4);
	if(ret < 0)	
	{
	...
	stk3x1x_obj->psoff_ir = word_data;
	...
	if(stk3x1x_obj->ges_enable_ir == 1)
	{
		if(word_data > 2000)
		{
			APS_LOG( "%s: word_data=%d, warning\n", __FUNCTION__, word_data);
			return 0xEE;
		}
	}
	if(word_data > lii)	
...
//	stk3x1x_read_als(obj->client, &data);
	...
	{
		obj->ps_high_thd_boot = obj->ps_stat_data[1] + 450;
		obj->ps_low_thd_boot = obj->ps_stat_data[1] + 400;	
		obj->boot_cali = 40;
	}
...
static int stk_ps_tune_zero_init(struct stk3x1x_priv *obj)
{
	...
	data = 0xff;
	err = stk3x1x_master_send(obj->client, 0x03, &data, 1);
	if (err < 0)
	{
		APS_ERR("%s write back ps_it err = %d\n", __func__,err);
		return -EFAULT;
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_priv *obj)
{
	...
	else if(ret == 0xFFFF)
	{
		...
		{
			obj->ps_high_thd_boot = obj->boot_ct + STK_HT_N_CT;
			obj->ps_low_thd_boot = obj->boot_ct + STK_LT_N_CT;
			atomic_set(&obj->ps_high_thd_val, obj->ps_high_thd_boot);
			atomic_set(&obj->ps_low_thd_val, obj->ps_low_thd_boot);
		}
		else
		...
		APS_LOG("%s: ret = %d, boot HT=%d, LT=%d, boot_cali=%d, sun=%d\n", __func__, ret, obj->ps_high_thd_boot, obj->ps_low_thd_boot, obj->boot_cali, obj->is_sun);
		...
		{
			data = 0xff;
			ret = stk3x1x_master_send(obj->client, 0x0a, &data, 1);
			if (ret < 0)
			{
				APS_ERR("%s write back ps_it ret = %d\n", __func__,ret);
				return -EFAULT;
			} 
			data = 0xff;
			ret = stk3x1x_master_send(obj->client, 0x0b, &data, 1);
			if (ret < 0)
			{
				APS_ERR("%s write back ps_it ret = %d\n", __func__,ret);
				return -EFAULT;
			...
			data = 0x00;
			ret = stk3x1x_master_send(obj->client, 0x0c, &data, 1);
			if (ret < 0)
			{
				APS_ERR("%s write back ps_it ret = %d\n", __func__,ret);
				return -EFAULT;
			...
			data = 0x1E;
			ret = stk3x1x_master_send(obj->client, 0x0d, &data, 1);
			if (ret < 0)
			{
				APS_ERR("%s write back ps_it ret = %d\n", __func__,ret);
				return -EFAULT;
	...
	if(diff == obj->psi)
	{
		...
		obj->ps_high_thd_boot = obj->psi + 320;
		obj->ps_low_thd_boot = obj->psi + 300;
		...
		if(atomic_read(&obj->ps_high_thd_val) > obj->ps_high_thd_tmp)
		{
			...
			if(obj->boot_cali >= 1 && obj->boot_ct != 0xFFFF)
			{
				obj->ps_high_thd_boot = obj->boot_ct + 320;
				obj->ps_low_thd_boot = obj->boot_ct + 300;	
		...
		*/
		...
		{
			obj->boot_ct = obj->psi;
			obj->ps_high_thd_tmp = obj->boot_ct + 450;
			obj->ps_low_thd_tmp = obj->boot_ct + 400;
...
static void stk3x1x_eint_work(struct work_struct *work)
{
	...
	stk3x1x_read_als(obj->client, &data1);
	stk3x1x_read_ps(obj->client, &data2);

	APS_LOG(" &obj->pending_intr =%lx, %d,%d,%d\n",obj->pending_intr, data1, obj->ps_nf, data2);

	if(((1<<1) & obj->pending_intr))
	{
		//get raw data
		disable_flag |= 0x20;
	}
	...
	if(obj->is_sun == 1)
	{
		...
		stk3x1x_ps_val();
		APS_LOG("stk ps_ir = %d, sun=%d\n", obj->psoff_ir, obj->is_sun);
	
		if(((data1 <= 30) || (obj->ps >= obj->ps_high_thd_boot) || (obj->ps_near == 1)) && ((flag_reg & STK_FLG_NF_MASK)==0) && (ps_int == 1)){
			sensor_data.values[0] = (flag_reg & STK_FLG_NF_MASK)? 1 : 0;

			data = 0x0;
			err = stk3x1x_master_send(obj->client, 0x0a, &data, 1);
			if (err < 0)
			{
				APS_ERR("%s write back ps_it err = %d\n", __func__,err);
				return -EFAULT;
			...
			data = 0x32;
			err = stk3x1x_master_send(obj->client, 0x0b, &data, 1);
			if (err < 0)
			{
				APS_ERR("%s write back ps_it err = %d\n", __func__,err);
				return -EFAULT;
			...
			data = 0x0;
			err = stk3x1x_master_send(obj->client, 0x0c, &data, 1);
			if (err < 0)
			{
				APS_ERR("%s write back ps_it err = %d\n", __func__,err);
				return -EFAULT;
			...
			data = 0x0;
			err = stk3x1x_master_send(obj->client, 0x0d, &data, 1);
			if (err < 0)
			{
				APS_ERR("%s write back ps_it err = %d\n", __func__,err);
				return -EFAULT;
			...
			if(obj->ps_near == 1)
			{
				...
				APS_LOG("%s:ps high = 0x%x\n",__FUNCTION__, atomic_read(&obj->ps_high_thd_val));
			}
			
			if((obj->psi == 0xFFFF) && (obj->ps_nf == 1))
			{
				if((err = stk3x1x_write_ps_high_thd(obj->client, (obj->ps))))
				{
					APS_ERR("write high thd error: %d\n", err);
					return err;        
				}
				
				if ((err = stk3x1x_write_ps_low_thd(obj->client, (obj->ps-50))))
				{
					APS_ERR("write low thd error: %d\n", err);
					return err;
				}
				APS_LOG("%s:ps low = 0x%x, high = 0x%x\n",__FUNCTION__, (obj->ps-50), (obj->ps));
		...
		else if(((data1 <= 30) || (obj->ps >= obj->ps_high_thd_boot)) && ((obj->psi == 0xFFFF) || ((flag_reg & STK_FLG_NF_MASK)==0)) && (ps_int == 0) && (obj->ps_nf == 1))
		{
			obj->ps_near = 1;
			obj->ps_int_re = 1;
			data = 0x00;
			err = stk3x1x_master_send(obj->client, 0x04, &data, 1);
			if (err < 0)
			{
				APS_ERR("%s write back ps_it err = %d\n", __func__,err);
				return -EFAULT;
			...
			if((err = stk3x1x_write_ps_high_thd(obj->client, 0x0)))
			{
				APS_ERR("write high thd error: %d\n", err);
				return err;        
			}
			...
			if((obj->psi == 0xFFFF) && (obj->ps_nf == 1))
			{
				...
				APS_LOG("%s:ps low = 0x%x\n",__FUNCTION__, (obj->ps-50));
			}
			APS_LOG("%s: re_int, %d\n", __func__, obj->ps_high_thd_boot);
		}
		else if((data1 < 10) && (obj->psoff_ir < 10) && (obj->ps_nf == 0) && (obj->psi == 0xFFFF) && ((flag_reg & STK_FLG_NF_MASK)==0) && (obj->ps_near == 2))
		{
			obj->ps_near = 0;
			...
			if((err = stk3x1x_write_ps_high_thd(obj->client, (obj->ps+30))))
			{
				APS_ERR("write high thd error: %d\n", err);
				return err;        
			}
				
			if ((err = stk3x1x_write_ps_low_thd(obj->client, (obj->ps-30))))
			{
				APS_ERR("write low thd error: %d\n", err);
				return err;
			}
			APS_LOG("%s:ps low = 0x%x, ,ps high = 0x%x, ps = 0x%x\n",__FUNCTION__, (obj->ps-30), (obj->ps+30), obj->ps);
		}
		else if(((data1 >= 50) && (obj->ps < obj->ps_low_thd_boot)) || ((flag_reg & STK_FLG_NF_MASK)==1))
		{
			sensor_data.values[0] = 1;
			obj->ps_nf = sensor_data.values[0];
			obj->ps_int_re = 1;
			...
			if((obj->ps < obj->psi) && (obj->psoff_ir < 100) && ((obj->ps+STK_LT_N_CT) < STK_LT_DEF))
			{
				obj->psi = obj->ps;
				obj->ps_high_thd_boot = obj->ps + STK_HT_N_CT;
				obj->ps_low_thd_boot = obj->ps + STK_LT_N_CT;
				if(obj->ps_high_thd_boot > STK_HT_DEF)
				{
					obj->ps_high_thd_boot = STK_HT_DEF;
					obj->ps_low_thd_boot = STK_LT_DEF;
				...
				if(obj->psi < obj->boot_ct)
				{
					obj->boot_ct = obj->psi;
					obj->boot_cali = obj->boot_cali + 1;
					obj->ps_high_thd_tmp = obj->boot_ct + 450;
					obj->ps_low_thd_tmp = obj->boot_ct + 400;
					if(obj->ps_high_thd_tmp > STK_HT_DEF)
					{
			...
			APS_LOG("%s:ps low = 0x%x, high = 0x%x, cali=%d\n",__FUNCTION__, atomic_read(&obj->ps_low_thd_val), atomic_read(&obj->ps_high_thd_val), obj->boot_cali);
	...
	else
	{
		...
		if(((data1 <= 20) || (obj->ps >= obj->ps_high_thd_boot) || (obj->ps_near == 1)) && ((flag_reg & STK_FLG_NF_MASK)==0) && (ps_int == 1))
		{
			sensor_data.values[0] = (flag_reg & STK_FLG_NF_MASK)? 1 : 0;
			obj->ps_nf = sensor_data.values[0];
			data = 0x0;
		...
		else if(((data1 <= 20) || (obj->ps >= obj->ps_high_thd_boot)) && ((flag_reg & STK_FLG_NF_MASK)==0) && (ps_int == 0) && (obj->ps_nf == 1))
		{
			obj->ps_near = 1;
			obj->ps_int_re = 1;
			data = 0x00;
		...
		else if(((data1 >= 20) && (obj->ps < obj->ps_low_thd_boot)) || ((flag_reg & STK_FLG_NF_MASK)==1))
		{
			sensor_data.values[0] = 1;
			obj->ps_nf = sensor_data.values[0];
			obj->ps_int_re = 1;
	...
	if((obj->boot_cali <= 20) && (obj->boot_ct < STK_LT_DEF) && (obj->ps_nf == 0) && (obj->psi_set == 0) && (obj->psi != 0xFFFF) && (obj->is_sun == 0))
	{
...
/* Gionee liyang 2014-6-27 for Psensor begin */
int stk3x1x_get_noise(struct i2c_client *client, int *ps_noise, int len)
{
	int err = 0, i = 0;
	int retry = 0;
	u16 __read_data = 0;
	...
	if (err = stk3x1x_enable_ps(client, 1, 1)) {
		APS_ERR("stk3x1x ioctl disable ps fail: %d\n", err);
		goto out;
	}

	for (i = 0, retry = 0; i < len; i++, retry++) {
		if (err = stk3x1x_read_ps(client, &__read_data)) {
				i--;
				if((len * 2) == retry)
					break;
				continue;
		}
		*(ps_noise + i) = __read_data;
		APS_LOG(" get stk3x1x ps_noise = %x\n", *(ps_noise + i));
		msleep(50);
	}
...
/* Gionee liyang 2014-6-27 for Psensor end */
...
#endif
{
	...
	int ps_cali;
        int threshold[2];
/* Gionee chengyj modify it for meta extend at 2014-6-24 begin */
#if defined(GN_META_EXTEND_SUPPORT)
	int ps_noise[8]={0} ;
	int i = 0, retry = 0;
#endif
/* Gionee chengyj modify it for meta extend at 2014-6-24 end */
	...
	switch (cmd)
	{
		...
		case ALSPS_GET_ALS_DATA: 
			...
			if(obj->als < 16)	//als = 0~15 report 0 lux
			{
				obj->als_last = 0;
...
/* Gionee chengyj modify it for meta extend at 2014-6-24 begin */
		...
		case ALSPS_GET_PS_NOISE:
#if 1
			if(err = stk3x1x_get_noise(obj->client, &ps_noise[0], sizeof(ps_noise) / sizeof(ps_noise[0]))){
				err = -EFAULT;
				goto err_out;
			}
#else
			...
			for (i = 0, retry = 0; i < 8; i++, retry++) {
				if (err = stk3x1x_read_ps(obj->client, &ps_noise[i])) {
						i--;
						if((8 * 2) == retry)
							break;
						continue;
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
			...
			//Gionee chengyj 2013-11-29 add for Devices information check tool start
#ifdef GN_MTK_BSP_DEVICECHECK
		case SENSOR_GET_CPIF:
			if(buff_out == NULL)
			{
				APS_ERR("get sensor data parameter error!\n");
				err = -EINVAL;
			...
			{
				snprintf(buff_out, size_out, "%s:%s", stk3x1x_DEV_NAME, "stk3410");
			}
			break;
#endif
			//Gionee chengyj 2013-11-29 add for Devices information check tool end	
...
static int stk_run_calibration(struct stk3x1x_priv *data) 
{
	struct stk3x1x_priv *obj = data;	
	u8 reg;	
	int err = 0;
	int rst = -1;
	int org_state_reg = atomic_read(&obj->state_val);
...
*/
	...
	reg = STK_STATE_EN_WAIT_MASK | STK_STATE_EN_PS_MASK; 
	err = stk3x1x_write_state(obj->client, reg);
	if(err < 0)
		goto err_out;
...
//#endif		
	...
	ps_hthr = obj->ps_cali +  obj->stk_ht_n_ct;
			
	ps_lthr = obj->ps_cali +  obj->stk_lt_n_ct;

	ps_len = sprintf(ps_calibration, "%d,%d", ps_hthr, ps_lthr);

	if(err = write_factory_calibration(obj, ps_calibration, ps_len))
	   {
...
static ssize_t alsps_calibration_read(struct file *f, char __user *buf, size_t count, loff_t *pos)
{
	int data_array[3] = {-1,-1,-1};
  struct stk3x1x_priv *obj = stk3x1x_obj;
    printk("alsps_calibration_read\n");

	data_array[0] = stk_run_calibration(obj) ;
	printk("the value of data is ==%d\n",data_array[0]);
	data_array[1] = data_array[0]+PS_h_offset; 
	data_array[2] = data_array[0]+PS_l_offset;
	if((data_array[0]>PS_MAX_XTALK)||(data_array[0]<0))
	data_array[0] = -1;
    printk("alsps_calibration_read,data0:%d,data1:%d,data2:%d,data3:%d\n",data_array[0],data_array[1],data_array[2],data_array[3]);
	if(copy_to_user(buf, &data_array, sizeof(data_array)))
		return -EFAULT;
...
{
	printk("alsps_calibration_open\n");
	return 0;
}
static int alsps_calibration_release(struct inode *inode, struct file *f)
{
	printk("alsps_calibration_release\n");
	return 0;
}
static int alsps_calibration_write(struct file *f, char __user *buf, size_t count, loff_t *pos)
{
	printk("alsps_calibration_write\n");
	return count;
}
...
#if STK_WRITE_CALI_PROC
static int alsps_calibration_proc_read(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
	int data_array[3] = {-1,-1,-1};
  	struct stk3x1x_priv *obj = stk3x1x_obj;
    printk("alsps_calibration_read\n");
	...
	data_array[0] = -1;
    printk("alsps_calibration_read,data0:%d,data1:%d,data2:%d,data3:%d\n",data_array[0],data_array[1],data_array[2],data_array[3]);
	if(copy_to_user(buffer, &data_array, sizeof(data_array)))
		return -EFAULT;

	return count;
}
static int alsps_calibration_proc_write(struct file *f, char __user *buf, size_t count, loff_t *pos)
{
	printk("alsps_calibration_write\n");
	return count;
}

static const struct file_operations alsps_calibration_proc_struct = { 
    .read = alsps_calibration_proc_read,
    .write = alsps_calibration_proc_write
};
#endif
...
static int stk3x1x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	{
		printk("Creat alsps_calibration_struct device file error!!\n");
		goto exit_misc_device_register_failed_cali;
	}
	else
	{
		printk("Creat alsps_calibration_struct device file success!!\n");
		}
#endif
	...
	obj->stk_lt_n_ct = STK_LT_N_CT;
	obj->stk_ht_n_ct = STK_HT_N_CT;
	 stk_cali_proc = proc_create(STK_CALI_PROC_FILE, 0444, NULL, &alsps_calibration_proc_struct);
	    if (stk_cali_proc == NULL)
	    {
...
#if STK_WRITE_CALI	
exit_misc_device_register_failed_cali:
	misc_deregister(&stk3x1x_device);	
#endif	
...
static int stk3x1x_i2c_remove(struct i2c_client *client)
{
	...
	input_free_device(obj->ges_input_dev);		
#endif
	if(err = stk3x1x_delete_attr(&stk3x1x_alsps_driver.driver))
	{
...
{
	struct mag_hw *hw = stk_get_cust_alsps_hw();
