...
#if CONFIG_MTK_DEV_INFO
int g_main_sensor_id=0;
int g_sub_sensor_id=0;
int get_camera_SensorID(int *main_sensorID,int *sub_sensorID)
{
    *main_sensorID = g_main_sensor_id;
    *sub_sensorID = g_sub_sensor_id;
    return 0;
}
EXPORT_SYMBOL(get_camera_SensorID);	
#endif
/*******************************************************************************
...
inline static int  adopt_CAMERA_HW_FeatureControl(void *pBuf)
{
	...
	case SENSOR_FEATURE_GET_SENSOR_CURRENT_TEMPERATURE:
	 ...
	 case  SENSOR_FEATURE_SET_RAMA_INFO:
	 		printk("SENSOR_FEATURE_SET_RAMA_INFO\n");
			 if(copy_from_user((void*)pFeaturePara , (void *) pFeatureCtrl->pFeaturePara, FeatureParaLen)) {
                kfree(pFeaturePara);
                PK_DBG("[CAMERA_HW][pFeaturePara] ioctl copy from user failed\n");
                return -EFAULT;
